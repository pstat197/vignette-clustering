...
#Step 5: Apply Cluster Labels to Original Dataset To actually add cluster labels to each observation in our dataset, we can use the `cutree()` method to cut the dendrogram into 4 clusters:
...
library(tidymodels)
library(cluster)
wine2 <- read_csv('clean-whitequality.csv')
```
