...
```{r}
gap_stat <- clusGap(winecut, FUN = hcut, nstart = 25, K.max = 10, B = 50)
fviz_gap_stat(gap_stat)
```


We observe that the gap statistic is highest at $k=1$ and $k=2$Thus, we select $k=2$ clusters to divide our dataset.

#Step 5: Apply Cluster Labels to the Dataset and add cluster labels to each observation in our dataset, we can use the `cutree()` method to cut the dendrogram into 3 clusters:


```{r}
d <- dist(winecut, method = "euclidean")
final_clust <- hclust(d, method = "ward.D2" )
groups <- cutree(final_clust, k=2)
final_data <- cbind(winecut, cluster = groups)
head(final_data)
```
