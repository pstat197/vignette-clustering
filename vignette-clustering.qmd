...
Just in case you forgot, the goal of kmeans clustering is to decrease the inter-cluster variation as much as possible so that the members of each cluster are as similar as possible. Each cluster is represented by the mean of all point, known as the centroid. The standard way to do this is using the Hartigan-Wong algorithm summarized in the following steps:
